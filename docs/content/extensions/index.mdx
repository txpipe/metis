...
- use the `cardano-relay` and `cardano-block-producer` MCP catalog entries if your primary concern is Cardano relay or block producer operation
- use the `apex-fusion-relay` and `apex-fusion-block-producer` MCP catalog entries if you are running Prime or Vector workloads
