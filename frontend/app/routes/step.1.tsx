...
export default function WizardStep1() {
  ...
  const [utxos, setUtxos] = useState<{ txHash: string; index: number; }[]>([]);
  const { setBadgeStatus } = useOutletContext<WizardStepContext>();
  const [selectedUtxo, setSelectedUtxo] = useState<{ txHash: string; index: number; } | null>(null);
    ...
    <>
      ...
      <div className="flex justify-end mt-14">
        <Button
          ...
          onClick={() => {
            navigate('/step/2', { state: { selectedUtxo } });
          }}
        >Continue
        </Button>
      </div>
