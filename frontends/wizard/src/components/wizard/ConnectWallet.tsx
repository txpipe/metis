...
export function ConnectWallet({ variant, fullWidth, className }: Props) {
  ...
  const { isConnecting, connectWallet, connectedWalletDetails } = useWallet();

  if (connectedWalletDetails) {
    return (
      <Button type="button" variant={variant} fullWidth={fullWidth} className={clsx(className, 'border-[#2B2B2B]/24 cursor-default pointer-events-none')}>
  ...
  return (
    <Button type="button" variant={variant} fullWidth={fullWidth} className={className} loading={isConnecting} onClick={() => connectWallet('eternl')}>
      {!isConnecting
        ? (
          'Connect wallet'
        )
        : (
          <>
            Connecting wallet <SpinnerIcon className="animate-spin ml-2" />
          </>
        )}
    </Button>
  );
...
// return (
//   <Menu>
//     <MenuButton className={clsx(className, 'w-fit max-w-full')} disabled={isConnecting || !!connectedWalletDetails} as="div">
