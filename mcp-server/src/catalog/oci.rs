...
pub(super) async fn fetch_catalog_json(
    ...
    if descriptor.size.is_some_and(|size| size > max_bytes) {
        return Err(OciCatalogError::CatalogTooLarge {
...
) -> Result<OciManifest, OciCatalogError> {
    let response = client
        ...
        .await?
        .error_for_status()
        .map_err(OciCatalogError::HttpStatus)?;
...
fn select_catalog_descriptor(manifest: &OciManifest) -> Option<OciDescriptor> {
    manifest
        ...
        .find(|descriptor| descriptor.media_type == CATALOG_LAYER_MEDIA_TYPE)
        .or_else(|| {
            manifest
                .layers
                .iter()
...
fn verify_sha256_digest(expected: &str, payload: &[u8]) -> Result<(), OciCatalogError> {
    let Some(expected) = expected.strip_prefix("sha256:") else {
        return Err(OciCatalogError::UnsupportedDigest(expected.to_string()));
    };
    let actual = hex_lower(&Sha256::digest(payload));
    if actual != expected.to_ascii_lowercase() {
        return Err(OciCatalogError::DigestMismatch {
...
impl OciReference {
    fn parse(value: &str) -> Result<Self, OciCatalogError> {
        let value = value.trim();
        let Some(rest) = value.strip_prefix("oci://") else {
            return Err(OciCatalogError::InvalidReference(value.to_string()));
        };
        ...
        let (repository, reference) = if let Some((repository, digest)) = path.split_once('@') {
            ...
            return Err(OciCatalogError::MissingReference(value.to_string()));
        };
...
pub enum OciCatalogError {
    ...
    #[error("OCI catalog artifact does not contain a catalog JSON layer: {0}")]
...
mod tests {
    ...
    fn parses_tagged_oci_references() {
        let reference =
            OciReference::parse("oci://oci.supernode.store/extension-catalog:0.1.0").unwrap();

        assert_eq!(reference.registry, "oci.supernode.store");
        assert_eq!(reference.repository, "extension-catalog");
        assert_eq!(reference.reference, "0.1.0");
        assert_eq!(
            reference.manifest_url(),
            "https://oci.supernode.store/v2/extension-catalog/manifests/0.1.0"
        );
    }
    ...
    fn parses_digest_oci_references() {
        let reference =
            OciReference::parse("oci://oci.supernode.store/extension-catalog@sha256:abc123")
                .unwrap();
    ...
    #[test]
    fn rejects_oci_references_without_tag_or_digest() {
        let error = OciReference::parse("oci://oci.supernode.store/extension-catalog").unwrap_err();

        assert!(matches!(error, OciCatalogError::MissingReference(_)));
    }

    #[test]
    fn identifies_trusted_catalog_references() {
        assert!(
            is_trusted_catalog_reference("oci://oci.supernode.store/extension-catalog:0.1.0")
                .unwrap()
        );
        assert!(
            !is_trusted_catalog_reference("oci://evil.example/extension-catalog:0.1.0").unwrap()
        );
    }

    #[test]
    fn selects_catalog_media_type_before_plain_json() {
        let manifest = serde_json::from_value::<OciManifest>(json!({
            "layers": [
                ...
                {
                    "mediaType": CATALOG_LAYER_MEDIA_TYPE,
        ...
        let descriptor = select_catalog_descriptor(&manifest).unwrap();
    ...
    fn rejects_sha256_digest_mismatch() {
        ...
        assert!(matches!(error, OciCatalogError::DigestMismatch { .. }));
    }
}
