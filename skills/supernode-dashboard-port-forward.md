...
5. If the user asks to open a local tunnel, explain that MCP does not currently provide a long-running port-forward tool.
6. If operator-approved local access is explicitly allowed, identify the exact output and describe a scoped local port-forward as a fallback.

## Rules

- Prefer MCP outputs over local tunnels.
